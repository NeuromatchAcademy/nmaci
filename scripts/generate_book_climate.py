...
def update_file(d, prefix):
    for k, v in d.items():
        if k == "file":
            d[k] = prefix + v
        elif isinstance(d[k], dict):
            d[k] = update_file(d[k], prefix=prefix)
        elif isinstance(d[k], list):
            d[k] = [update_file(dd, prefix=prefix) for dd in d[k]]
    return d
...
def main():
    ...
    try:
        with open("tutorials/pythia/_toc.yml") as fh:
            precourse_toc = yaml.load(fh, Loader=yaml.FullLoader)
    except:
        precourse_toc = None

    # Make the dictionary that contains the chapters
    toc = {}
    if precourse_toc is not None:
        toc["Pre-Course"] = {"part": [update_file(
            part, "tutorials/pythia/") for part in precourse_toc['parts'] if part.get('caption') != "Appendix"]}
    ...
    for m in materials:
        ...
        chapter = {
            "file": f"tutorials/{directory}/chapter_title.md",
        ...
        notebook_list = []
        notebook_list += (
            [f"{directory}/{ARG}/{m['day']}_Intro.ipynb"]
        ...
        notebook_list += (
            [f"{directory}/{ARG}/{m['day']}_Outro.ipynb"]
    ...
    #     # Add README guide
    #     this_section["sections"].append(
    #         {"file": f"projects/{category}/README.md", "title": "Guide"}
    #     )

    #     # Add and process all notebooks
    #     try:
    #         this_section["sections"].append(
    #             {"file": f"projects/{category}/{category}_videos.ipynb"}
    #         )
    ...
    # toc[part]["chapters"].append(
    #     {
    #         "file": "projects/docs/project_2020_highlights.md",
    #         "sections": [
    ...
    for i_cell, cell in enumerate(cells):
        ...
        if "@title" in source or "@markdown" in source:
            ...
            strings_with_markdown = [
                (i, string)
                for i, string in enumerate(cell["source"])
...
def change_video_widths(content):
    ...
    for cell in content["cells"]:
        ...
        elif len(cell["source"]) > 1 and "IFrame" in cell["source"][1]:
